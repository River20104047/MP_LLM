function [SNR,XYbc,XYbcnr,CI99] = Function_DSW_f05(XY,lbws,wsf,ck)
...
% 2024/03/27 v5: add window size factor (wsf)

%% Iput data and parameters
% ck      = 12; % For checking
% XY      = Tsmp{:,1:20}; Spectrum data
...
% Input parameters - may used for optimazation
% lbws1  = 25;       % small window size and stepsize for baseline correction, this is also used for calculating envelope
lbws1  = lbws;
lbws2  = wsf*lbws1;  % large window size and stepsize for baseline correction (factor of 5 was empirical, at prilimilary trials, we found 25 was good. 5*25=125 ~ 128 was from Renner et al., 2016, then factor of 5 was used to lower the number of parameters)

% Input parameters - empirical
...
BCfinal = BCnp+BCpk;
% BCfinal = sgolayfilt(BCnp+BCpk,2,2*floor(fs * lbws2 / abs(X(2) - X(1))/2)+1); % Smoothing size is a bit larger than large window size x1.5
...
XYbcnr = [X Ybcnr]; % noise removed & baseline corrected spectrum

% SNR estimation
sigma = fsk*(Rn.* (fup + flw) ./ (1.96*2)); % Here, Rn* (fup + flw) is 95% CI, but since it is not symmetric, cant use the relation between CI and sigma to estimate sigma
Hpeak = max(Ybcnr);
...
if ck > 0
    ...
    % plot(X,BC1(:,ck),'lineWidth',0.5,'Color','#CC99FF') % potential BC (small)
    % plot(X,BC2(:,ck),'lineWidth',0.5,'Color','#CC99FF') % potential BC (large)
    ...
    plot([min(X) max(X)],[0-ex*flw*0.5*Rn(ck) 0-ex*flw*0.5*Rn(ck)],'lineWidth',0.5,'Color','#FF7C80') % 0.7 = 1.1/1.5, 1.1 is for lower bound, 1.5 is for upper bound of 99% CI
    % plot(X,BCnp(:,ck),'lineWidth',1.2,'Color','#FF7C80')
    % plot(X,BCpk(:,ck),'lineWidth',1.2,'Color','#00CC99')
    ...
    set(gcf, 'Position', [1            1       1/2*2194.3 * 3/4      1234.3  * 3/4])
end
